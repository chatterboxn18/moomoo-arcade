...
public class LWPopupController : LWBaseController
{
	...
	public void Evt_OpenPopup()
	{
		...
		if (LWData.current.FlowerDictionary.ContainsKey(date.Month + "/" + date.Year) && LWData.current.FlowerDictionary[date.Month + "/" + date.Year][date.Day - 1].PlantIndex != -1)
	...
	public void ButtonEvt_Edit()
	{
		...
		LWTransitionController.TransitionTo(LWTransitionController.Controllers.Pot, LWTransitionController.Controllers.Water);
	}
