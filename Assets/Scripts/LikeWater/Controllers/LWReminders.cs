...
namespace LikeWater
{
	public class LWReminders : MonoBehaviour
	{
		...
		private void CreateReminder(LWData.NotificationData reminder, int counter)
		{
			...
			reminderItem.SetReminder(reminder.Time, counter, reminder.Ids.ToArray(), reminder.IsActive);
		...
		public void ButtonEvt_Delete()
		{
			...
			for (var i = _selectedIndex + 1; i < _reminderItems.Count; i++)
			{
				_reminderItems[_selectedIndex].SetIndex(i - 1);
			}
		}
	...
	public void ButtonEvt_SelectWeek(int index)
		...
		public void ButtonEvt_Add(int index)
		{
			...
			if (index == 0 && secondCharacter > 4 && character == 1)
				character = 0;
			else if (index == 1 && character == 4 && firstCharacter == 2)
				character = 0;
		...
		public void ButtonEvt_Minus(int index)
		{
			...
			else if (character == 0 && index == 1 && firstCharacter == 2) character = 4;
